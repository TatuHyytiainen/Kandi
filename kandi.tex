...
Yllä $a_i$ ja $b_i$ ovat muuttujan $\xi_i$ maalijoukon ylä- ja alarajat. Valitsemalla $t = 2s(m-s)(p-q) = \mathbb{E}[\Delta_{[A:B]}]$ voidaan arvioida yläraja tapahtuman $\Delta_{[A:B]} \leq 0$ todennäköisyydelle. Tämä vastaa sitä, että kiinteällä $m$ jaolla joukkoihin $A$ ja $B$, joilla $|A_+| = |B_-| = s$, olisi pienempi leikkausjoukko, kuin jaolla todellisiin yhteisöihin $C_-$ ja $C_+$.
\begin{align*}
...
\end{align*}
Täten verkon $G_f(n,p,q)$ minimoiva puolitus vastaa todellista yhteisöjakoa eksponentiaalisen pientä todennäköisyyttä lukuun ottamatta verkon koon kasvaessa rajatta.
\end{proof}
...
Teoreemat 2 ja 3 liittyvät parametrien $a$ ja $b$ arvoihin määäräämin rajoihin, joiden mukaan positiivisesti korreloitunut tunnistus joko on tai ei ole asymptoottisesti mahdollista riippuen parametreista $a$ ja $b$.
