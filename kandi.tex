...
\end{korollaari}
\begin{korollaari}
Kun $k = o(\sqrt{n})$, pätee $\mathbb{E}[X_{k_n}] = \frac{1}{2k_n}(d^{k_n} + f^{k_n}) \cdot (1 + o(1))$.
\end{korollaari}
Korollaarit 1 ja 2 seuraavat suoraan kun lemmojen 2 ja 3 tulokset yhdistetään.
\begin{korollaari}
Kun $k = O(\log^{1/4})$, pätee $\mathbb{E}[X_{k_n}] = \frac{1}{2k_n}\big(d^{k_n} + f^{k_n} + o(f^{k_n}) \big)$.
\end{korollaari}
\begin{proof}
	...
	\begin{align*}
		\mathbb{E}X_k = \frac{1}{2k} \big(d^k + f^k) \cdot (1 + O(\frac{\log^{1/2}n}{n})).
	\end{align*}
	KESKEN KESKEN
	KESKEN KESKEN
	KESKEN KESKEN
...
\begin{propositio}
	Olkoon $(a-b)^2 > 2(a+b)$ ja $\sqrt{\frac{a+b}{2}} < \rho < \frac{a-b}{2}$. Tällöin pätee $\text{Var}(X_{k_n}) = o(\rho^{2{k_n}})$, kun $k_n \rightarrow \infty$.
\end{propositio}
\begin{proof}
...
Merkitään $\hat{a}$ ja $\hat{b}$ seuraavia estimaattoreita parametreille $a$ ja $b$. Estimaattorit $\hat{a}$ ja $\hat{b}$ voidaan laskea $\hat{d}$:n ja $\hat{f}$:n avulla siten että
	...
	\begin{align*}
		...
		&= \frac{c}{{n^{2 \beta}} +  O(1)} + o(n^{2\beta}) \\
