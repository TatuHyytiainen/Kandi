...
%%
\thesistitle{Yhteisöjen tunnistaminen satunnaisissa verkoissa}
...
%%
\section{Johdanto}
Tässä työssä perehdytään yhteisöntunnistukseen satunnaisverkoissa, tarkemmin stokastisessa lohkomallissa. Käsiteltävissä satunnaisverkoissa verkon jokainen solmu kuuluu johonkin yhteisöön ja solmujen yhteisörakenne vaikuttaa siihen, kuinka satunnaisverkko muodostuu. Yhteisöntunnistuksella tarkoitetaan tässä verkon solmujen yhteisörakenteen täydellistä tai osittaista tunnistamista vailla esitietoa minkään solmun yhteisöstä.

Yhteisöntunnistus on kiinnostava tutkimuskohde matematiikan lisäksi verkostotieteessä ja monissa sen sovellusaloissa. Viimeaikainen verkostodatan lisääntynyt määrä on kasvattanut kiinnostusta ongelmaa kohtaan. Työssä esitellään lyhyesti yhteisöntunnistuksen sovelluskohteita.

Työssä osoitetaan yhteisöntunnistuksen olevan helpompi tehtävä verkoissa, joissa kaaria on tiheästi verrattuna harvoihin verkkoihin. Kummassakin tapauksessa käsitellään asymptoottisia tuloksia. Harvassa tapauksessa osoitetaan selvä raja stokastisen lohkomallin parametreille, milloin yhteisöntunnistus on asymptoottisesti mahdollista ja milloin ei.

Työn viimeisessä luvussa tutkitaan yhteisöntunnistusta laskennallisesti simuloiduissa verkoissa. Kahta eri yhteisöntunnistusalgoritmia vertaillaan simulaatioissa ja tuloksia verrataan teorian osoittamiin tunnistettavuusrajoihin.
...
Yllä mainitussa karateklubin verkossa on 34 solmua ja se onkin esimerkki yhdestä ensimmäisestä yhteisöntunnistustutkimuksesta. Nykyisin monissa tutkittavissa verkoissa on vähintään tuhansia ja usein jopa miljoonia solmuja, joten asymptoottisten matemaattisten tulosten käyttäminen on hyvin perusteltua.

Yhteisöntunnistusongelmaa ja yhteisöntunnistusta sovelluksissa on käsitelty yksityiskohtaisemmin esimerkiksi artikkelissa \cite{Fortunato}.
...
Yhteisöjen tunnistaminen on helpompaa stokastisen lohkomallin tiheässä tapauksessa. Tässä luvussa näytetään, että yhteisöt voidaan tiheässä tapauksessa tunnistaa suurella todennäköisyydellä täydellisesti verkon koon kasvaessa äärettömäksi.
...
\begin{proof}[Lemman \ref{lemma:kombinaatiokertoma_nk} todistus.]
	...
	\end{align*}
	kun $k = \text{o}(\sqrt{n})$. Koska lausekkeen ala- ja yläraja lähestyvät arvoa 1 kun $n$ kasvaa, niin $\binom{n}{k}\frac{k!}{n^{k}} \xrightarrow{n \to \infty} 1$.
\end{proof}
%\begin{lemma}
...
Merkitään $\hat{a}$ ja $\hat{b}$ seuraavia estimaattoreita parametreille $a$ ja $b$. Estimaattorit $\hat{a}$ ja $\hat{b}$ voidaan laskea $\hat{d}$:n ja $\hat{f}$:n avulla siten että
	...
	Esitetään $\hat{d}^{k_n}$ eksponenttifunktion ja logaritmin avulla:
	\begin{align*}
	...
	\end{align*}
	Käytetään nyt Tšebyšovin epäyhtälöä arvioimaan $\frac{Z_{k_n}}{f^{k_n}}$:n poikkeamaa odotusarvostaan:
	\begin{align*}
...
Eräs menetelmä, jota voidaan käyttää yhteisöjen tunnistamiseen stokastisessa lohkomallissa perustuu verkon naapuruusmatriisin spektriin. Siinä yhteisöjaon estimaattorina toimivat naapuruusmatriisin toiseksi suurinta ominaisarvoa vastaavan ominaisvektorin alkioiden merkit. Koska kyseessä on suuntaamaton verkko, sen naapuruusmatriisi on symmetrinen, ja täten sen ominaisarvot ovat reaaliarvoisia. Kunkin solmun yhteisön estimaattorina toimii yksinkertaisesti ominaisvektorissa sitä vastaavan alkion merkki siten, että kaikki positiivisen ja negatiivisen merkin omaavia alkioita vastaavat solmut jaetaan eri yhteisöihin. Tämä yhteisöjako estimoi todellisia yhteisöjä usein hyvin.
...
Taaksekääntymättömällä operaattorilla pyritään estimoimaan solmujen merkkejä ${B}$:n ominaisarvoja käyttämällä. Matriisi ${B}$:n suurinta ominaisarvoa $\lambda_1$ vastaavan ominaisvektorin $\mathbf{v}_{\lambda_1}$ alkioiden avulla estimointi tehdään seuraavasti: kuhunkin solmuun saapuvia kaaria vastaavat ominaisvektorin alkiot summataan, ja tämän summan merkkiä käytetään estimoimaan solmun merkkiä.
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Parametrin $a$ muutos vakio $b$:lla ja kiinteällä $n$:llä. Tunnistaminen alkaa onnistua kun suhde $\frac{(a-b)^2}{2(a+b)}$ saavuttaa arvon yksi ja tunnistaminen onnistuu aina paremmin suhteen kasvaessa. Vasemmanpuoleisessa kuvassa verkon koko on 1000 ja oikeanpuoleisessa 2000 solmua.}
	\label{fig:a_param}
\end{figure}
